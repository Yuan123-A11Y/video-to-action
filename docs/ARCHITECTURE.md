...
| **前端框架** | React 18 + TypeScript | Web UI（frontend/ 目录） |
| **前端构建** | Vite | 快速热更新 + 生产构建 |
| **样式框架** | Tailwind CSS | 响应式设计 |
| **视频下载** | yt-dlp | 多平台视频下载 |
| **音频处理** | ffmpeg | 音频提取、关键帧截取 |
| **语音转写** | faster-whisper | 音频转文字（支持中文） |
| **LLM分析** | OpenAI API / Ollama / LM Studio | 视频内容理解与方案生成 |
...
- 支持 dry-run 模式（仅打印命令不执行）
- 安全边界检查（危险操作需确认）
- 执行结果验证
- 多层安全检查（黑名单、白名单、交互式工具检测、超时保护）

**安全边界**：
- 运行远程脚本（`curl | bash` 等）需确认
- 输入密码/API Key 需确认
- 修改系统环境变量需确认
- 安装系统级软件需确认
- 检测到交互式工具（Claude、Cursor 等）时跳过
- 命令执行超时自动终止（默认 300 秒）

**实现细节**：
- 使用 `shlex.split()` 拆分命令参数，`shell=False` 执行（避免命令注入）
- 危险命令黑名单：`FORBIDDEN_KEYWORDS`
- 需确认操作白名单：`REQUIRE_CONFIRM`
- 交互式工具检测：`INTERACTIVE_TOOLS` 集合
...
- [x] ✅ 完善 Executor 模块，支持多层安全检查（已完成）
- [x] ✅ 添加 Web UI 界面（已完成 — React + TypeScript + Vite）
- [x] ✅ 支持多种 LLM 提供商（已完成 — OpenAI / Ollama / LM Studio）
- [ ] 支持更多视频平台（快手、小红书等）
- [ ] 优化 LLM 分析 Prompt，提高准确率
- [ ] 批量处理支持（CSV/Excel 导入）
- [ ] 添加多语言支持
