...
class FfmpegLocator:
    ...
    async def _probe(self) -> None:
        ...
        try:
            ...
            stdout, _stderr = await asyncio.wait_for(proc.communicate(), timeout=_VERSION_PROBE_TIMEOUT_SECONDS)
        ...
        if proc.returncode != 0 or b"ffmpeg version" not in stdout.lower():
            ...
            self._version = None
            self._available = False
            self._last_error = f"ffmpeg -version exit={proc.returncode}, " f"stdout_head={stdout[:64]!r}"
            return
