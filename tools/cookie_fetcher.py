...
使用方法：
  ...
  # 获取B站Cookie（Netscape格式，用于yt-dlp）
  python tools/cookie_fetcher.py --platform bilibili --format netscape
...
PLATFORMS = {
    ...
    "douyin": {
        ...
        "domain": "douyin.com",
        "required_keys": {"sessionid", "sid_tt", "odin_tt"},
        "suggested_keys": {"sessionid", "sid_tt", "sid_guard", "odin_tt", "ttwid", "passport_csrf_token"},
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="json",
        help="输出格式：json (默认) 或 netscape (用于yt-dlp)",
    )
...
async def capture_cookies(args: argparse.Namespace) -> int:
    ...
    async with async_playwright() as p:
        ...
        # 从浏览器存储状态提取Cookie（完整信息）
        print("\n[INFO] 正在提取Cookie...")
        storage = await context.storage_state()
        
        # 提取完整Cookie信息（用于Netscape格式）
        full_cookies = [
            cookie for cookie in storage["cookies"]
    ...
    if args.format == "netscape":
        # Netscape格式（用于yt-dlp）
        netscape_file = output.with_suffix(".txt") if output.suffix == ".json" else output
        save_cookies_netscape(full_cookies, netscape_file)
        print(f"\n✅ 已保存 {len(full_cookies)} 个Cookie到（Netscape格式）：{netscape_file.resolve()}")
        print(f"   可在yt-dlp中使用：--cookies {netscape_file}")
    else:
        # JSON格式（默认）
        output.write_text(json.dumps(picked, ensure_ascii=False, indent=2), encoding="utf-8")
        print(f"\n✅ 已保存 {len(picked)} 个Cookie到（JSON格式）：{output.resolve()}")
...
def save_cookies_netscape(cookies: list, output_file: Path) -> None:
    """保存Cookie为Netscape格式（用于yt-dlp）"""
    with open(output_file, "w", encoding="utf-8") as f:
        # Netscape格式文件头
        f.write("# Netscape HTTP Cookie File\n")
        f.write("# This file was generated by cookie_fetcher.py\n")
        f.write("# Usage: yt-dlp --cookies " + str(output_file) + "\n\n")
